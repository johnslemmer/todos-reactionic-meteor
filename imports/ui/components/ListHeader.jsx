...
export default class ListHeader extends React.Component {
  ...
  deleteList() {
    ...
    if (confirm(message)) { // eslint-disable-line no-alert
      remove.call({ listId: list._id }, this.context.popError);
  ...
  toggleListPrivacy() {
    const list = this.props.list;
    if (list.userId) {
      makePublic.call({ listId: list._id }, this.context.popError);
    } else {
      makePrivate.call({ listId: list._id }, this.context.popError);
    }
  }
  ...
  render() {
    const { list } = this.props;
    return (
      <IonSubHeaderBar {...this.props}>
        <h2 className="title">{list.name} COUNT:{list.incompleteCount}</h2>
      </IonSubHeaderBar>
    );
  }
}
